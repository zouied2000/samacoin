...
#if defined(NDEBUG)
# error "Samacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0e5b2cf3d976cc733fe7297bc3c7f9e951f23dcffbeea5b27ff4c814ecc64940");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Samacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Samacoin Signed Message:\n";
...
{
    // Samacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Samacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Samacoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Samacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Samacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Samacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0e5b2cf3d976cc733fe7297bc3c7f9e951f23dcffbeea5b27ff4c814ecc64940");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        //     CTxOut(nValue=50.00000000, scriptPubKey=0495e5bea0d046ba47d7bf00e71369)
        //   vMerkleTree: 97ddfbbae6

        // Genesis block
        const char* pszTimestamp = "20 out 1955 this is the first step!!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ff049057b73fc1404b7d3cd87bf0c3c0d3ada91f16deda76bb640640cf0fa3bfd7a606de7195653112988df3ce984d8597ad278322e8c7acc0538444d5d45b39") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1503233218;
        ...
        assert(block.hashMerkleRoot == uint256("0xd6d484408bc0e667e5247a654f257346b8bc4ed040da36b0b6c1a579355a57a0"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Samacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SamacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SamacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SamacoinMiner(CWallet *pwallet)
{
    printf("SamacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("samacoin-miner");
    ...
    try { loop {
        ...
        printf("Running SamacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SamacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SamacoinMiner, pwallet));
}
